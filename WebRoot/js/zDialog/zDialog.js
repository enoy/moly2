        ...
        return "";
          ...
          <tr id="_ButtonRow_{thisID}" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_{thisID}" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" value="' + this.OkButtonText + '" class="but" id="_ButtonOK_{thisID}"/>\
                <input type="button" value="' + this.CancelButtonText + '" class="but" onclick="Dialog.getInstance(\'{thisID}\').close();" id="_ButtonCancel_{thisID}"/>\
              </div></td>\
          </tr>\
        </table></td>\
...
Dialog.getBgdiv = function () {
    ...
    if (!bgdiv) {
		...
		var bgIframeMask = '<div id="_DialogBGMask" style="position:absolute;background-color:#333;opacity:0.4;filter:alpha(opacity=40);width:1000%;height:100%;"></div>';
...
//底部按钮为是和否
Dialog.confirmYS = function (msg, funcOK, funcCal, w, h) {
    var w = w || 300,
        h = h || 110;
    var diag = new Dialog({
        Width: w,
