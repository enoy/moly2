<@p.header4content navTitle="回收站" toggleSearch=true>
<@p.includeDatePicker/>
</@p.header4content>

<form action="${base}SysNetdisk/recycle.html" id="listform" method="post">
	<@ui.searchBar>
	<tr>
	...
	<@ui.ctrlBar>
	<input type="button" value="清空回收站" class="but" onclick="gotoUrl('${base}SysNetdisk/emptyRecycle.html')"/>
	</@ui.ctrlBar>
	...
	<tr>
		...
		<td>
			...
			&nbsp;|&nbsp;
			<a href = "${base}SysNetdisk/restore.html?id=${u.id}" title="恢复">恢复</a>
			&nbsp;|&nbsp;
			<a href = "#" title="彻底删除" onclick="confirmDelete('${base}SysNetdisk/forceDelete.html?id=${u.id}')">彻底删除</a>
		</td>
	</tr>
	</#list>
	</#if>
